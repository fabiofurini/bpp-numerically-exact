...
Reading the figure: all 66 bins are shown, 11 per row, in the same order
the solver reports them. Each bin is a column as tall as the capacity
(2456); inside it, every item the solver placed there is a colored block
stacked from the bottom, as tall as that item's weight and labeled with
the item's index from the instance file. A bin's blocks always sum to at
most the capacity line at the top of the column — the packing constraint
the solver has to satisfy for every one of the 66 columns at once.
...
[Input](docs/INPUT.md)). The last few bins (5 of 66, visible as the denser
